...
{
  {
    ...
    L"del X    - Delete file X            shutdown - Shutdown PC (-r reboot)\n"
    L"\x0000"
  },
  {
...
RtlClipProcessMessage(PCHAR Command)
{
    ...
    // If drive is not "C", we run into problems. 
    // TODO: fix them.
    PCWSTR NativePath = L"C:\\Windows\\System32\\native.exe";
    ...
    strncpy(CommandBuf, Command, _strnlen(Command, BUFFER_SIZE));
    ...
    else if (!_strnicmp(Command, "shutdown -r", 11))
    {
      RtlCliReboot();
    }
     ...
     ULONG DebugFlag OPTIONAL)
{
    ...
    RtlCliDisplayString("(C) Copyright 2022 sidhys \n");
